...
export const tournamentMatchDefinition = baseModelDefinition.extend({
  ...
  rightTeam: z
    ...
    .or(z.string().refine(isValidObjectId))
    .optional(),
  score: z.array(z.array(z.number()).length(2)),
...
export const getTournamentMatchesQueryDefinition =
  tournamentMatchDefinition.partial();
export type TGetTournamentMatchesQueryDefinition = z.infer<
  typeof getTournamentMatchesQueryDefinition
>;

export const getTournamentMatchParamsDefinition =
  tournamentMatchDefinition.pick({ _id: true });
export type TGetTournamentMatchParamsDefinition = z.infer<
  typeof getTournamentMatchParamsDefinition
>;

export const createTournamentMatchInputDefinition = tournamentMatchDefinition
  .omit({
    _id: true,
  })
  .refine(
    (data) => {
      if (data.eventType === "1V1") {
        if (
          data.leftPlayer &&
          data.rightPlayer &&
          data.leftPlayer !== data.rightPlayer
        ) {
          return true;
        }
    ...
    {
      message:
        "Registre todos los jugadores/equipos del partido y asegúrese de que no sean los mismos",
...
export const updateTournamentMatchParamsDefinition =
  tournamentMatchDefinition.pick({
    _id: true,
  });
...
export const updateTournamentMatchInputDefinition =
  tournamentMatchDefinition.partial();
